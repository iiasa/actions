...
if [$INSTALLER] == 'anaconda'; then
  case $RUNNER_OS in
    Linux)
      ...
      CONDA_OS=Linux
      FRAGMENT=$VERSION_ANACONDA-${CONDA_OS}-x86_64.sh
      ;;
    macOS)
      CACHE_PATH="$GITHUB_ACTION_PATH/conda.pkg"
      CONDA_OS=MacOSX
      FRAGMENT=$VERSION_ANACONDA-${CONDA_OS}-x86_64.pkg
      ;;
    Windows)
      CACHE_PATH="$GHA_PATH\\conda.exe"
      CONDA_OS=Windows
      FRAGMENT=$VERSION_ANACONDA-${CONDA_OS}-x86_64.exe
      ;;
  esac
...
if [$INSTALLER] == 'miniconda'; then
  case $RUNNER_OS in
    Linux)
      ...
      CONDA_OS=Linux
      FRAGMENT=$VERSION_MINICONDA-${CONDA_OS}-x86_64.sh
      ;;
    MacOS)
      PATH_ENDING = conda.pkg
      CACHE_PATH="$GITHUB_ACTION_PATH/$PATH_ENDING"
      CONDA_OS=MacOSX
      FRAGMENT=$VERSION_MINICONDA-${CONDA_OS}-x86_64.pkg
      ;;
    Windows)
      PATH_ENDING = conda.exe
      CACHE_PATH="$GHA_PATH\\$PATH_ENDING"
      CONDA_OS=Windows
      FRAGMENT=$VERSION_MINICONDA-${CONDA_OS}-x86_64.exe
      ;;
  esac
